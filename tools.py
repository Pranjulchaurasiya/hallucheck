...
def _default_web_search(query: str, num_results: int = 10) -> Dict[str, Any]:
    client = SerperClient(mock=os.getenv("MOCK_SERPER", "false").lower() == "true")
    return client.web_search(query, num_results)


def _default_news_search(query: str) -> Dict[str, Any]:
    client = SerperClient(mock=os.getenv("MOCK_SERPER", "false").lower() == "true")
    return client.news_search(query)


# Tool implementation registry mapping tool_name -> callable
TOOL_IMPLEMENTATIONS: Dict[str, Any] = {
    "web_search": _default_web_search,
    "news_search": _default_news_search,
}


def bind_serper_client(client: "SerperClient") -> None:
    """Bind a specific SerperClient instance's search methods to TOOL_IMPLEMENTATIONS."""
    TOOL_IMPLEMENTATIONS["web_search"] = client.web_search
    TOOL_IMPLEMENTATIONS["news_search"] = client.news_search
...
def register_tool(
    ...
    """Register a new tool schema and implementation for the harness.

    Enables adding custom tools without modifying harness or core runner code.
    """
    name = schema["function"]["name"]
    existing_idx = next((i for i, s in enumerate(TOOL_SCHEMAS) if s.get("function", {}).get("name") == name), None)
    if existing_idx is not None:
        TOOL_SCHEMAS[existing_idx] = schema
    else:
...
class SerperClient:
    ...
    def __init__(self, api_key: Optional[str] = None, mock: bool = False):
        ...
        # Bind Serper instance methods into global implementations registry
        TOOL_IMPLEMENTATIONS["web_search"] = self.web_search
        TOOL_IMPLEMENTATIONS["news_search"] = self.news_search
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if response.status_code >= 400:
                    logger.warning(f"Serper HTTP {response.status_code} ({response.text}) for payload: {payload}")
    ...
    def web_search(self, query: str, num_results: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if "Query pattern not allowed" in str(e):
                logger.info("Retrying Serper query without num parameter...")
                payload_fallback = {"q": query}
                data = self._execute_with_retry("search", payload_fallback)
            else:
                raise
