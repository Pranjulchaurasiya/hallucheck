...
from tools import TOOL_SCHEMAS, SerperClient

logger = logging.getLogger(__name__)

INVESTIGATE_SYSTEM_PROMPT = """You are a research analyst investigating how a stealth marketing/distribution
agency executes product launches on X and LinkedIn. You have web_search and
...
SYNTHESIS_SYSTEM_PROMPT = """You are synthesizing research findings across multiple product launches run
by the same distribution agency. You will be given per-client findings. Your
...
def execute_tool_call(
    ...
    try:
        if tool_name == "web_search":
            num_results = parsed_args.get("num_results", 10)
            res = serper_client.web_search(parsed_args["query"], num_results=num_results)
            return res, None
        elif tool_name == "news_search":
            res = serper_client.news_search(parsed_args["query"])
            return res, None
        else:
            err_msg = f"Tool call error: Unknown tool '{tool_name}'. Available tools: web_search, news_search."
            return {"error": err_msg}, err_msg
    except Exception as e:
...
def run_client_investigation(
    ...
    for turn in range(1, max_turns + 1):
        ...
        # Determine if tool calling is allowed by budget
        tools_available = TOOL_SCHEMAS if budget.can_consume() else None

        # If budget exhausted and we haven't warned LLM yet
        if budget.is_exhausted() and turn > 1 and not any("budget limit reached" in str(m.get("content", "")).lower() for m in messages):
            messages.append({
                "role": "system",
                "content": "Budget limit reached. You can no longer make tool calls. Please formulate your final structured summary based solely on the data gathered so far."
            })
        ...
        for tc in tool_calls:
            ...
            # Execute tool safely
            raw_result, error_str = execute_tool_call(func_name, raw_args, serper_client)
            ...
            if error_str:
                ...
                tool_content = json.dumps(raw_result, ensure_ascii=False)
...
def run_synthesis(
    ...
    for client_name, data in all_findings.items():
        summary = data.get("summary", "No summary recorded")
